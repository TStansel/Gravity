...
export const lambdaHandler = async (
  ...
  try {
    let verifiedEvent = Question.verifyCreateEvent(event as unknown as JSON);
    ...
    let createEvent = verifiedEvent.value;

    let createResult = await createEvent.create();
    if (createResult.type === "error") {
      customLog(createResult.error.message, "ERROR");
      return buildResponse(500, createResult.error.message);
    }
  } catch (error) {
